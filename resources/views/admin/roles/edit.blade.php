...
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">Modify role name and grant/revoke module capabilities.</p>
        </div>
        <a href="{{ route('admin.roles.index') }}" class="px-3 py-1.5 rounded-xl bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-xs font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition">
            ← Back to Roles
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 sm:p-8 shadow-xs">
        ...
        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="space-y-8">
            ...
            <!-- Role Name -->
            <div class="max-w-md">
                <label for="name" class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Role Name</label>
                <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" required
                    {{ $role->name === 'Super Admin' ? 'readonly' : '' }}
                    class="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500 transition {{ $role->name === 'Super Admin' ? 'opacity-75 cursor-not-allowed' : '' }}">
                @if ($role->name === 'Super Admin')
                    <span class="text-[11px] text-amber-600 dark:text-amber-400 mt-1 block">Super Admin role title is protected by system integrity.</span>
                @endif
            </div>

            <!-- Permission Matrix Toolbar & Search Filter -->
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3.5">
                    <div>
                        <h3 class="font-bold text-slate-900 dark:text-white text-sm font-heading">Assign Permissions by Module</h3>
                        <p class="text-xs text-slate-400">Permissions are grouped logically. Filter or toggle capabilities below.</p>
                    </div>
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <div class="relative w-full sm:w-48">
                            <input type="text" x-model="searchFilter" placeholder="Filter permissions..."
                                class="w-full pl-8 pr-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <button type="button" @click="toggleAll(true)" class="px-3 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 text-xs font-bold shrink-0 transition">
                            Check All
                        </button>
                        <button type="button" @click="toggleAll(false)" class="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 text-xs font-bold shrink-0 transition">
                            Clear
                        </button>
                    </div>
                </div>
                ...
                <div class="space-y-5">
                    @foreach ($groupedPermissions as $module => $permissions)
                        ...
                        <div x-data="{
                        ...
                        }" x-init="updateCount()" class="p-5 rounded-2xl bg-slate-50/80 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-800">
                            ...
                            <div class="flex items-center justify-between mb-3 pb-2 border-b border-slate-200/60 dark:border-slate-700/60">
                                <div class="flex items-center gap-2">
                                    <span class="text-sm">{{ $moduleIcon }}</span>
