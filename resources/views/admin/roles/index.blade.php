...
<div x-data="{ inspectModalOpen: false, selectedRole: null }" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        @forelse ($roles as $role)
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 shadow-xs flex flex-col justify-between hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-200">
                <div>
                    <!-- Header -->
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-2xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white font-bold text-base flex items-center justify-center shadow-md shadow-purple-500/20">
                                🛡️
                            </div>
                            <div>
                        ...
                        @if ($role->name === 'Super Admin')
                            <span class="px-2.5 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/80 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 text-[10px] font-bold tracking-wider uppercase">
                                Protected
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-5">
                        <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                            <span class="text-[10px] text-slate-400 font-bold uppercase block">Users</span>
                            <span class="text-xl font-black font-heading text-slate-900 dark:text-white">{{ number_format($role->users_count) }}</span>
                        </div>
                        <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                            <span class="text-[10px] text-slate-400 font-bold uppercase block">Permissions</span>
                            <span class="text-xl font-black font-heading text-indigo-600 dark:text-indigo-400">
                    ...
                    <div class="mb-4">
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-2">Module Coverage</span>
                ...
                <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    ...
                    ]) }}; inspectModalOpen = true" class="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1">
                    ...
                    <div class="flex items-center gap-1.5">
                        @can('roles.edit')
                            <a href="{{ route('admin.roles.edit', $role) }}" class="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 text-xs font-semibold transition">
                                Edit
                            </a>
                        @endcan
                        ...
                        @can('roles.delete')
                            @if ($role->name !== 'Super Admin')
                                <form method="POST" action="{{ route('admin.roles.destroy', $role) }}" onsubmit="return confirm('Are you sure you want to delete role {{ $role->name }}?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-3 py-1.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-rose-700 dark:text-rose-300 text-xs font-semibold transition">
                                        Delete
                                    </button>
                                </form>
                            @endif
    ...
    <!-- Inspect Role Modal -->
    <div x-show="inspectModalOpen" x-cloak class="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4">
        <div @click.away="inspectModalOpen = false" class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 max-w-2xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-slate-900 dark:text-white text-base font-heading" x-text="selectedRole?.name"></h3>
                        <p class="text-xs text-slate-400">Granted Spatie capabilities breakdown</p>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-y-auto space-y-4 pr-1">
                <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 text-xs flex justify-between text-slate-600 dark:text-slate-300 font-medium">
                ...
                <div class="space-y-3">
                    <template x-for="(perms, mod) in selectedRole?.module_breakdown" :key="mod">
                        <div class="p-4 rounded-2xl bg-slate-50/80 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-800">
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                <template x-for="p in selectedRole?.permissions" :key="p">
                                    <span x-show="p.startsWith(mod.toLowerCase() + '.')" class="px-2 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 font-mono text-[11px] text-slate-800 dark:text-slate-200" x-text="p"></span>
                                </template>
                            </div>
                        </div>
                    </template>
