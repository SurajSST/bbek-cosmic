...
<div class="max-w-5xl mx-auto space-y-6" x-data="{ fileName: '', dragging: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-xl font-extrabold font-heading text-slate-900 dark:text-white flex items-center gap-2">
                <svg class="w-6 h-6 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path>
                </svg>
                Bulk Upload Sales Orders
            </h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Import multiple Sales Orders and product items simultaneously using an Excel or CSV spreadsheet.</p>
        </div>
        <div class="flex items-center gap-2.5">
            <a href="{{ route('admin.sales-orders.bulk-upload.sample') }}" class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 transition shadow-2xs">
    ...
    @if (session('bulk_errors') && count(session('bulk_errors')) > 0)
        <div class="p-5 rounded-3xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 text-xs space-y-3">
            <div class="flex items-center gap-2 font-bold text-sm text-amber-800 dark:text-amber-300">
                ...
                Some Sales Orders could not be processed ({{ count(session('bulk_errors')) }} issues):
            </div>
            <div class="max-h-60 overflow-y-auto space-y-1.5 pr-2 font-mono text-[11px]">
                @foreach (session('bulk_errors') as $bulkErr)
                    <div class="p-2 rounded-xl bg-amber-100/60 dark:bg-amber-900/40 border border-amber-200/60 dark:border-amber-800/60">
                        {{ $bulkErr }}
                    </div>
                @endforeach
            </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 sm:p-8 shadow-xs">
        <form method="POST" action="{{ route('admin.sales-orders.bulk-upload.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                >
                    ...
                    <div class="flex flex-col items-center justify-center space-y-3">
                        <div class="w-14 h-14 rounded-2xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center group-hover:scale-110 transition duration-200">
                        ...
                        <div>
                            <p class="text-sm font-bold text-slate-900 dark:text-white">
                                <span class="text-indigo-600 dark:text-indigo-400 underline">Click to browse</span> or drag and drop your spreadsheet here
                            </p>
                            <p class="text-xs text-slate-400 mt-1">Supports CSV or Excel spreadsheets up to 10MB</p>
                        </div>

                        <div x-show="fileName" x-cloak class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-indigo-100 dark:bg-indigo-950/60 text-indigo-800 dark:text-indigo-200 text-xs font-mono font-bold border border-indigo-200 dark:border-indigo-800">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-xl text-xs font-bold bg-indigo-600 text-white hover:bg-indigo-500 transition shadow-md shadow-indigo-600/25 flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 sm:p-8 space-y-5 shadow-xs">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3.5">
            <h3 class="font-bold text-slate-900 dark:text-white text-sm flex items-center gap-2 font-heading">
                ...
                Excel / CSV Column Structure Guide
            </h3>
        ...
        <div class="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
            <p>
                <strong class="text-slate-900 dark:text-white">Multi-item grouping rule:</strong>
                If multiple rows in your spreadsheet contain the exact same <code class="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-mono font-bold">so_number</code>, they are automatically merged into a single Sales Order with multiple line items.
            </p>
        </div>
        ...
        <div class="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800">
            <table class="w-full text-left text-xs">
                <thead class="bg-slate-50/80 dark:bg-slate-950/80 text-slate-700 dark:text-slate-300 font-bold uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
                    <tr>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-slate-600 dark:text-slate-300 font-mono">
                    <tr class="bg-slate-50/30 dark:bg-slate-900/30">
                        ...
                        <td class="px-4 py-2.5 font-sans">Unique Sales Order number (e.g. <code class="font-mono">SO-2026-001</code>). Grouping identifier.</td>
                    </tr>
                    <tr>
                        <td class="px-4 py-2.5 font-bold text-indigo-600 dark:text-indigo-400">so_from</td>
                        <td class="px-4 py-2.5 text-rose-500 font-sans font-bold">Yes</td>
                        <td class="px-4 py-2.5 text-slate-500 font-sans">String</td>
                        <td class="px-4 py-2.5 font-sans">Source entity (e.g. <code class="font-mono">Cloud</code>, <code class="font-mono">Dragon</code>, <code class="font-mono">Cosmic</code>).</td>
                    </tr>
                    <tr class="bg-slate-50/30 dark:bg-slate-900/30">
                        ...
                        <td class="px-4 py-2.5 text-slate-500 font-sans">String</td>
                        <td class="px-4 py-2.5 font-sans">Entity issuing the bill (defaults to <code class="font-mono">so_from</code> if omitted).</td>
                    </tr>
                    <tr>
                        <td class="px-4 py-2.5 font-bold text-slate-700 dark:text-slate-300">billed_to</td>
                        <td class="px-4 py-2.5 text-slate-400 font-sans">Optional</td>
                        <td class="px-4 py-2.5 text-slate-500 font-sans">String</td>
                        <td class="px-4 py-2.5 font-sans">Customer / Client receiving the bill (e.g. <code class="font-mono">PBS</code>, <code class="font-mono">EGA</code>).</td>
                    </tr>
                    <tr>
                        ...
                        <td class="px-4 py-2.5 font-sans"><code class="font-mono">pending</code>, <code class="font-mono">billed</code>, <code class="font-mono">paid</code>, or <code class="font-mono">cancelled</code>.</td>
                    </tr>
                    <tr class="bg-slate-50/30 dark:bg-slate-900/30">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2.5 text-rose-500 font-sans font-bold">Yes</td>
                        <td class="px-4 py-2.5 text-slate-500 font-sans">String</td>
                        <td class="px-4 py-2.5 font-sans">Product / maintenance name (e.g. <code class="font-mono">Server Cloud Plan</code>).</td>
                    </tr>
                    <tr class="bg-slate-50/30 dark:bg-slate-900/30">
                        ...
                        <td class="px-4 py-2.5 font-sans">Quantity (default: 1).</td>
                    </tr>
                    <tr>
                        ...
                        <td class="px-4 py-2.5 font-sans">Price per unit (default: 0.00). Total is auto-calculated.</td>
                    </tr>
                </tbody>
            </table>
        </div>
