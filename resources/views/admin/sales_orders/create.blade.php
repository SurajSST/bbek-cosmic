...
@section('header', 'Create Sales Order')
...
<div class="max-w-5xl mx-auto space-y-6" x-data="{
    ...
    addItem() {
        this.items.push({ product_name: '', quantity: 1, unit_price: 0, remarks: '' });
    },
    removeItem(index) {
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold font-heading text-slate-900 dark:text-white flex items-center gap-2">
                <svg class="w-6 h-6 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                New Sales Order
            </h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Record a new sales order with multiple products/services and upload bill/slip receipts.</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 sm:p-8 shadow-xs">
        ...
        <form method="POST" action="{{ route('admin.sales-orders.store') }}" enctype="multipart/form-data" class="space-y-8">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    <div>
                        ...
                        <select id="billed_status" name="billed_status" required
                            ...
                            <option value="pending" {{ old('billed_status') === 'pending' ? 'selected' : '' }}>Pending</option>
                            <option value="billed" {{ old('billed_status', 'billed') === 'billed' ? 'selected' : '' }}>Billed</option>
                            <option value="paid" {{ old('billed_status') === 'paid' ? 'selected' : '' }}>Paid</option>
                            <option value="cancelled" {{ old('billed_status') === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="bill_no" name="bill_no" value="{{ old('bill_no') }}" placeholder="e.g. INV-2026-001"
                    ...
                    <!-- Bill Image File -->
                    <div>
                        <label for="bill_image" class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Bill Image / Receipt</label>
                    ...
                    <!-- Slip Image File -->
                    <div>
                        <label for="slip_image" class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Slip Image / Proof</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                    ...
                    <div>
                        ...
                        <textarea id="remarks" name="remarks" rows="2" placeholder="Brief internal order notes..."
                            class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('remarks') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="2" placeholder="Detailed terms or breakdown..."
                            class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description') }}</textarea>
                    </div>
                </div>
            </div>
