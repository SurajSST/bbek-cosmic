...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">Add a new user account to the system and assign security roles.</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 sm:p-8 shadow-xs">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Active (Enabled)</option>
            ...
            <!-- Role Assignment -->
            <div>
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2.5">Assign Spatie Roles</label>
