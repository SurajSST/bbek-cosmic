...
<div x-data="{ detailsModalOpen: false, modalTitle: '', modalProperties: null, modalIp: '', modalAgent: '' }" class="space-y-6">
    ...
    <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs">
        <form method="GET" action="{{ route('admin.activity-logs.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3">
            ...
            <div class="sm:col-span-3">
                <select name="action" class="w-full px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
            ...
            <div class="sm:col-span-3 flex gap-2">
                ...
                @if (request('search') || (request('action') && request('action') !== 'all'))
                    <a href="{{ route('admin.activity-logs.index') }}" class="px-3 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold transition flex items-center justify-center" title="Reset Filters">
                        ✕
                    </a>
                @endif
            </div>
                    ...
                    @forelse ($activityLogs as $index => $log)
                        <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition">
                            ...
                            <td class="py-3.5 px-4">
                                <div class="font-bold text-slate-900 dark:text-slate-100 text-xs">{{ $log->user_name }}</div>
                                @if($log->user)
                            ...
                            <td class="py-3.5 px-4 text-center">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border {{ $actionStyle }}">
                                    {{ str_replace('_', ' ', $log->action) }}
                                </span>
                            </td>

                            <!-- Description -->
                            <td class="py-3.5 px-4 font-medium text-slate-800 dark:text-slate-200 max-w-sm truncate text-xs">
                                {{ $log->description }}
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-right">
                                <button @click="modalTitle = 'Log Entry Details #{{ $log->id }}'; modalProperties = {{ json_encode($log->properties) }}; modalIp = '{{ $log->ip_address }}'; modalAgent = '{{ addslashes($log->user_agent) }}'; detailsModalOpen = true"
                                    class="px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/60 text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 font-bold text-[11px] transition shadow-2xs">
                                    Context
                                </button>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="8" class="py-12 text-center text-slate-400">
                                <div class="max-w-xs mx-auto space-y-2">
                                    ...
                                    <p class="font-bold text-slate-700 dark:text-slate-300">No Activity Logs found</p>
                                    <p class="text-xs text-slate-400">Activities will be recorded automatically during operations.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <!-- Details Modal -->
    <div x-show="detailsModalOpen" x-cloak class="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4">
        <div @click.away="detailsModalOpen = false" class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 max-w-xl w-full p-6 shadow-2xl space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 class="font-bold text-slate-900 dark:text-white text-base font-heading" x-text="modalTitle"></h3>
            ...
            <div class="space-y-3 text-xs">
                <div>
                    <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Client User Agent</span>
                    <p class="font-mono text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 break-all text-[11px]" x-text="modalAgent || 'N/A'"></p>
                </div>

                <div>
                    <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Properties & Context Payload</span>
                    <pre class="font-mono text-emerald-400 bg-slate-950 p-3.5 rounded-2xl border border-slate-800 overflow-x-auto max-h-60 text-[11px]" x-text="modalProperties ? JSON.stringify(modalProperties, null, 2) : 'No extra properties attached.'"></pre>
                </div>
            </div>
