...
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    ...
    billedFromPreset: ['Cloud', 'Dragon', 'Cosmic'].includes('{{ $uploadSo->billed_from }}') ? '{{ $uploadSo->billed_from }}' : 'Other',
    billedFromCustom: '{{ $uploadSo->billed_from }}',
    billedToPreset: ['PBS', 'Prativa Plus Two', 'Prativa School', 'EGA'].includes('{{ $uploadSo->billed_to }}') ? '{{ $uploadSo->billed_to }}' : 'Other',
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" id="amount" name="amount" value="{{ old('amount', $uploadSo->amount) }}" placeholder="0.00"
                            class="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
            </div>
