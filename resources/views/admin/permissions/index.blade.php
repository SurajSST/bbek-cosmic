...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-xl font-bold font-heading text-slate-900 dark:text-white flex items-center gap-2">
                <svg class="w-6 h-6 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path></svg>
                Permission Registry
            </h2>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
            ...
            <form method="GET" action="{{ route('admin.permissions.index') }}" class="relative min-w-[240px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search permissions..."
                    class="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 shadow-xs transition">
                <svg class="w-4 h-4 text-slate-400 absolute left-3 top-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            </form>

            @can('permissions.create')
                <button onclick="document.getElementById('createPermissionModal').classList.remove('hidden')" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white text-xs font-bold shadow-md shadow-indigo-600/25 transition hover:scale-[1.02] active:scale-[0.98] shrink-0">
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-1 border-b border-slate-200/80 dark:border-slate-800">
        <a href="{{ route('admin.permissions.index', ['module' => 'all', 'search' => request('search')]) }}"
            class="px-3.5 py-2 rounded-xl text-xs font-bold shrink-0 transition-all flex items-center gap-2 {{ request('module', 'all') === 'all' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800' }}">
        ...
        @foreach ($modulesList as $modName => $count)
            <a href="{{ route('admin.permissions.index', ['module' => strtolower($modName), 'search' => request('search')]) }}"
                class="px-3.5 py-2 rounded-xl text-xs font-bold shrink-0 transition-all flex items-center gap-2 {{ strtolower(request('module')) === strtolower($modName) ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800' }}">
                <span>{{ $modName }}</span>
    ...
    <div class="space-y-6">
        @forelse ($groupedPermissions as $module => $modulePermissions)
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 p-6 shadow-xs">
                <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-[10px] text-slate-400 font-mono">Prefix: {{ strtolower($module) }}.*</span>
                        </div>
                    </div>
                    <span class="px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5">
                    @foreach ($modulePermissions as $perm)
                        ...
                        <div class="p-3.5 rounded-2xl bg-slate-50/80 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-800 flex flex-col justify-between space-y-3">
                            <div>
                                ...
                                <div class="flex flex-wrap gap-1 mt-2">
                                    @forelse ($perm->roles as $r)
                                        <span class="px-1.5 py-0.5 rounded-md bg-purple-50 dark:bg-purple-950/80 text-purple-700 dark:text-purple-300 text-[10px] font-bold border border-purple-200/60 dark:border-purple-800/60">
                                            {{ $r->name }}
                                        </span>
                                    @empty
                            ...
                            <div class="pt-2 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between text-[11px]">
                                @if ($isProtected)
                                    <span class="text-amber-600 dark:text-amber-400 font-semibold text-[10px]">Core System</span>
                                @else
                                    <span class="text-slate-400 text-[10px]">Custom Capability</span>
                                @endif
                                ...
                                @can('permissions.delete')
                                    @if (!$isProtected)
                                        <form method="POST" action="{{ route('admin.permissions.destroy', $perm) }}" onsubmit="return confirm('Delete permission {{ $perm->name }}?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-rose-600 dark:text-rose-400 hover:underline font-bold text-[10px]">
                                                Delete
                                            </button>
                                        </form>
                                    @endif
    ...
    <div id="createPermissionModal" class="hidden fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 max-w-md w-full p-6 shadow-2xl space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 class="font-bold text-slate-900 dark:text-white text-base font-heading">New Granular Permission</h3>
            ...
            <form method="POST" action="{{ route('admin.permissions.store') }}" class="space-y-4">
                ...
                <div x-data="{ isCustom: false }">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Target Module</label>
                ...
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Action Name</label>
                    <input type="text" name="action" required placeholder="e.g. view, create, edit, delete, repay, approve"
                        class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>

                <div class="p-3 rounded-2xl bg-indigo-50 dark:bg-indigo-950/50 text-[11px] text-indigo-700 dark:text-indigo-300">
                    Resulting convention: <span class="font-mono font-bold">module.action</span> (e.g. <span class="font-mono">sales-orders.bulk-upload</span>). Automatically granted to Super Admin.
                </div>
                ...
                <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
                    ...
                    <button type="submit" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-md shadow-indigo-600/25">
                        Create Permission
                    </button>
                </div>
            </form>
