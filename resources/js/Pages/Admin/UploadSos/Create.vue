...
function removeSoImage() {
    form.so_image = null;
    soImagePreview.value = null;
    if (soCameraInput.value) soCameraInput.value.value = '';
    if (soGalleryInput.value) soGalleryInput.value.value = '';
}
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="p-4 rounded-2xl bg-slate-50/80 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/80 space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <span v-if="form.so_image" class="text-[10px] font-bold text-emerald-500">✓ Attached</span>
                            </div>
                            ...
                            <div v-if="!soImagePreview" class="grid grid-cols-2 gap-2">
                                <button type="button" @click="soCameraInput.click()" class="p-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white flex flex-col items-center justify-center gap-1 transition active:scale-95 shadow-sm shadow-indigo-600/20">
                                    <span class="text-lg">📸</span>
                                    <span class="text-xs font-bold">Take Photo</span>
                                    <span class="text-[9px] opacity-80">Open Camera</span>
                                </button>
                                <button type="button" @click="soGalleryInput.click()" class="p-3.5 rounded-xl bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-100 flex flex-col items-center justify-center gap-1 transition active:scale-95">
                                    <span class="text-lg">📁</span>
                            ...
                            <div v-else class="space-y-2">
                                <div class="relative w-full h-44 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-900">
                                    ...
                                    <button type="button" @click="removeSoImage" class="absolute top-2 right-2 p-1.5 rounded-lg bg-rose-600 text-white text-xs hover:bg-rose-500 shadow-md transition" title="Remove Photo">
                                        🗑️ Remove
                                    </button>
                                </div>
                                <div class="flex items-center gap-2">
                                    <button type="button" @click="soCameraInput.click()" class="flex-1 py-1.5 px-3 rounded-lg bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 text-xs font-bold transition">
                                        📸 Retake
                                    </button>
                                    <button type="button" @click="soGalleryInput.click()" class="flex-1 py-1.5 px-3 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition">
                                        📁 Choose Other
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="p-4 rounded-2xl bg-slate-50/80 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/80 space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <span v-if="form.slip_image" class="text-[10px] font-bold text-emerald-500">✓ Attached</span>
                            </div>
                            ...
                            <div v-if="!slipImagePreview" class="grid grid-cols-2 gap-2">
                                ...
                                <button type="button" @click="slipGalleryInput.click()" class="p-3.5 rounded-xl bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-100 flex flex-col items-center justify-center gap-1 transition active:scale-95">
                                    <span class="text-lg">📁</span>
                                    <span class="text-xs font-bold">Choose File</span>
                                    <span class="text-[9px] text-slate-500 dark:text-slate-400">Photo Library</span>
                                </button>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <textarea v-model="form.description" rows="2" placeholder="Detailed notes..." class="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-900 dark:text-white"></textarea>
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                        <Link href="/admin/upload-sos" class="px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-xs font-bold text-slate-700 dark:text-slate-300">Cancel</Link>
                        <button type="submit" :disabled="form.processing" class="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-md shadow-indigo-600/30 transition disabled:opacity-50 flex items-center gap-2">
                            <span v-if="form.processing">Saving SO Slip...</span>
                            <span v-else>Save SO Slip &rarr;</span>
                        </button>
                    </div>
